...
//----------------------------------------------
// 27-055.1 (ERROR)
// Tab. 2.2: Rd.11 COL.2 ≠ 0  =>  Tab. 1.1.1: Rd.20 COL.1 ≠ 0
// (fără invers)

function validate27_055_1(values) {
    var col1 = "C1";
    var col2 = "C2";
    ...
    // NIVEL TOTAL
    // ======================
    var Rd11_Col2 = !isNaN(Number(values["CAP22_R11_" + col2]))
        ? Number(values["CAP22_R11_" + col2])
        : 0;
    ...
    // Rd.11 C2 ≠ 0 => Rd.20 C1 ≠ 0
    if (Rd11_Col2 !== 0 && Rd20_Col1 === 0) {
        webform.errors.push({
            fieldName: 'CAP111_R20_' + col1,
            weight: 19,
            msg: Drupal.t(
                'Cod eroare: 27-055.1. Dacă Tab. 2.2, Rând.11 COL.2 ≠ 0, atunci Tab. 1.1.1, Rând.20 COL.1 trebuie să fie ≠ 0. Valori: R11-C2(@r11), R20-C1(@r20)',
                { '@r11': Rd11_Col2, '@r20': Rd20_Col1 }
            )
        });
    }
    ...
    if (values.CAP_NUM_FILIAL) {
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            var Rd11_Col2_F =
                values["CAP22_R11_" + col2 + "_FILIAL"] &&
                    !isNaN(Number(values["CAP22_R11_" + col2 + "_FILIAL"][j]))
            ...
            if (Rd11_Col2_F !== 0 && Rd20_Col1_F === 0) {
                webform.errors.push({
                    ...
                    weight: 19,
                    msg: Drupal.t(
                        'Raion: @cuatm – Cod eroare: 27-055.1-F. Dacă Tab. 2.2, Rând.11 COL.2 ≠ 0, atunci Tab. 1.1.1, Rând.20 COL.1 trebuie să fie ≠ 0. Valori: R11-C2(@r11), R20-C1(@r20)',
                        { '@cuatm': CAP_CUATM_FILIAL, '@r11': Rd11_Col2_F, '@r20': Rd20_Col1_F }
                    )
                });
            }
        }
...
//----------------------------------------------
// 27-056.1 (ERROR)
// Tab. 1.1.1: Rd.21 COL.1  <=>  Tab. 1.1: Rd.3 COL.7
// (dacă unul ≠ 0 atunci și celălalt ≠ 0, și invers)

function validate27_056_1(values) {
    var c1 = "C1";
    var c7 = "C7";
    ...
    // NIVEL TOTAL
    // ======================
    var Rd21_C1 = !isNaN(Number(values["CAP111_R21_" + c1]))
        ? Number(values["CAP111_R21_" + c1])
        : 0;

    // Tab. 1.1, Rd.3, Col.7
    var Rd3_C7 = !isNaN(Number(values["CAP11_R3_" + c7]))
        ? Number(values["CAP11_R3_" + c7])
        : 0;

    // Rd.21 ≠ 0 => Rd.3 C7 ≠ 0
    if (Rd21_C1 !== 0 && Rd3_C7 === 0) {
        webform.errors.push({
            fieldName: 'CAP11_R3_' + c7,
            weight: 19,
            msg: Drupal.t(
                'Cod eroare: 27-056.1. Dacă Tab. 1.1.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1, Rând.3 COL.7 trebuie să fie ≠ 0. Valori: R21-C1(@r21), R3-C7(@r3)',
                { '@r21': Rd21_C1, '@r3': Rd3_C7 }
            )
        });
    }

    // Rd.3 C7 ≠ 0 => Rd.21 ≠ 0
    if (Rd3_C7 !== 0 && Rd21_C1 === 0) {
        webform.errors.push({
            fieldName: 'CAP111_R21_' + c1,
            weight: 19,
            msg: Drupal.t(
                'Cod eroare: 27-056.1. Dacă Tab. 1.1, Rând.3 COL.7 ≠ 0, atunci Tab. 1.1.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R3-C7(@r3), R21-C1(@r21)',
                { '@r3': Rd3_C7, '@r21': Rd21_C1 }
            )
        });
    }
    ...
    if (values.CAP_NUM_FILIAL) {
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            var cuatm = isNaN(String(values.CAP_CUATM_FILIAL[j])) ? "" : String(values.CAP_CUATM_FILIAL[j]);

            var Rd21_C1_F =
                values["CAP111_R21_" + c1 + "_FILIAL"] &&
                    !isNaN(Number(values["CAP111_R21_" + c1 + "_FILIAL"][j]))
                    ? Number(values["CAP111_R21_" + c1 + "_FILIAL"][j])
                    : 0;

            var Rd3_C7_F =
                values["CAP11_R3_" + c7 + "_FILIAL"] &&
                    !isNaN(Number(values["CAP11_R3_" + c7 + "_FILIAL"][j]))
                    ? Number(values["CAP11_R3_" + c7 + "_FILIAL"][j])
                    : 0;

            // Rd.21 ≠ 0 => Rd.3 C7 ≠ 0 (FILIAL)
            if (Rd21_C1_F !== 0 && Rd3_C7_F === 0) {
                webform.errors.push({
                    fieldName: 'CAP11_R3_' + c7 + '_FILIAL',
                    index: j,
                    weight: 19,
                    msg: Drupal.t(
                        'Raion: @cuatm – Cod eroare: 27-056.1-F. Dacă Tab. 1.1.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1, Rând.3 COL.7 trebuie să fie ≠ 0. Valori: R21-C1(@r21), R3-C7(@r3)',
                        { '@cuatm': cuatm, '@r21': Rd21_C1_F, '@r3': Rd3_C7_F }
                    )
                });
            }

            // Rd.3 C7 ≠ 0 => Rd.21 ≠ 0 (FILIAL)
            if (Rd3_C7_F !== 0 && Rd21_C1_F === 0) {
                webform.errors.push({
                    fieldName: 'CAP111_R21_' + c1 + '_FILIAL',
                    index: j,
                    weight: 19,
                    msg: Drupal.t(
                        'Raion: @cuatm – Cod eroare: 27-056.1-F. Dacă Tab. 1.1, Rând.3 COL.7 ≠ 0, atunci Tab. 1.1.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R3-C7(@r3), R21-C1(@r21)',
                        { '@cuatm': cuatm, '@r3': Rd3_C7_F, '@r21': Rd21_C1_F }
                    )
                });
            }
        }
...
//----------------------------------------------
// 27-075.1 (ERROR)
// Cap. 2.2: Rd.1 COL(1-12)  <=>  Rd.2 COL(1-12)
// Pentru fiecare coloană 1..12: dacă una ≠ 0 atunci și cealaltă ≠ 0 (și invers)
// + FILIALE

function validate27_075_1(values) {
    ...
    for (var c = 1; c <= 12; c++) {
        ...
        // NIVEL TOTAL
        // ======================
        var r1 = N(values["CAP22_R1_" + col]);
        var r2 = N(values["CAP22_R2_" + col]);
        ...
        if (r1 !== 0 && r2 === 0) {
            webform.errors.push({
                ...
                weight: 19,
                msg: Drupal.t(
                    "Cod eroare: 27-075.1. Cap.2.2: Dacă Rd.1 COL.@c ≠ 0, atunci Rd.2 COL.@c trebuie să fie ≠ 0. Valori: R1=@r1, R2=@r2",
                    { "@c": c, "@r1": r1, "@r2": r2 }
                )
            });
        }
        ...
        if (r2 !== 0 && r1 === 0) {
            webform.errors.push({
                ...
                weight: 19,
                msg: Drupal.t(
                    "Cod eroare: 27-075.1. Cap.2.2: Dacă Rd.2 COL.@c ≠ 0, atunci Rd.1 COL.@c trebuie să fie ≠ 0. Valori: R2=@r2, R1=@r1",
                    { "@c": c, "@r2": r2, "@r1": r1 }
                )
            });
        }
        ...
        if (values.CAP_NUM_FILIAL) {
            for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
                ...
                var r1f = N(values["CAP22_R1_" + col + "_FILIAL"] && values["CAP22_R1_" + col + "_FILIAL"][j]);
                var r2f = N(values["CAP22_R2_" + col + "_FILIAL"] && values["CAP22_R2_" + col + "_FILIAL"][j]);
                ...
                if (r1f !== 0 && r2f === 0) {
                    webform.errors.push({
                        ...
                        weight: 19,
                        msg: Drupal.t(
                            "Raion: @cuatm – Cod eroare: 27-075.1-F. Cap.2.2: Dacă Rd.1 COL.@c ≠ 0, atunci Rd.2 COL.@c trebuie să fie ≠ 0. Valori: R1=@r1, R2=@r2",
                            { "@cuatm": cuatm, "@c": c, "@r1": r1f, "@r2": r2f }
                        )
                    });
                }
                ...
                if (r2f !== 0 && r1f === 0) {
                    webform.errors.push({
                        ...
                        weight: 19,
                        msg: Drupal.t(
                            "Raion: @cuatm – Cod eroare: 27-075.1-F. Cap.2.2: Dacă Rd.2 COL.@c ≠ 0, atunci Rd.1 COL.@c trebuie să fie ≠ 0. Valori: R2=@r2, R1=@r1",
                            { "@cuatm": cuatm, "@c": c, "@r2": r2f, "@r1": r1f }
                        )
                    });
                }
            }
...
//----------------------------------------------
// 27-075.2 (ERROR)
// Cap. 2.2: Rd.4 COL(1-12)  <=>  Rd.5 COL(1-12)
// Pentru fiecare coloană 1..12: dacă una ≠ 0 atunci și cealaltă ≠ 0 (și invers)
// + FILIALE

function validate27_075_2(values) {
    ...
    for (var c = 1; c <= 12; c++) {
        ...
        // NIVEL TOTAL
        // ======================
        var r4 = N(values["CAP22_R4_" + col]);
        var r5 = N(values["CAP22_R5_" + col]);

        // r4 ≠ 0 => r5 ≠ 0
        if (r4 !== 0 && r5 === 0) {
            webform.errors.push({
                fieldName: "CAP22_R5_" + col,
                weight: 19,
                msg: Drupal.t(
                    "Cod eroare: 27-075.2. Cap.2.2: Dacă Rd.4 COL.@c ≠ 0, atunci Rd.5 COL.@c trebuie să fie ≠ 0. Valori: R4=@r4, R5=@r5",
                    { "@c": c, "@r4": r4, "@r5": r5 }
                )
            });
        }
        ...
        if (r5 !== 0 && r4 === 0) {
            webform.errors.push({
                fieldName: "CAP22_R4_" + col,
                weight: 19,
                msg: Drupal.t(
                    "Cod eroare: 27-075.2. Cap.2.2: Dacă Rd.5 COL.@c ≠ 0, atunci Rd.4 COL.@c trebuie să fie ≠ 0. Valori: R5=@r5, R4=@r4",
                    { "@c": c, "@r5": r5, "@r4": r4 }
                )
            });
        }
        ...
        if (values.CAP_NUM_FILIAL) {
            for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
                ...
                var r4f = N(values["CAP22_R4_" + col + "_FILIAL"] && values["CAP22_R4_" + col + "_FILIAL"][j]);
                var r5f = N(values["CAP22_R5_" + col + "_FILIAL"] && values["CAP22_R5_" + col + "_FILIAL"][j]);

                // r4 ≠ 0 => r5 ≠ 0 (FILIAL)
                if (r4f !== 0 && r5f === 0) {
                    webform.errors.push({
                        fieldName: "CAP22_R5_" + col + "_FILIAL",
                        index: j,
                        weight: 19,
                        msg: Drupal.t(
                            "Raion: @cuatm – Cod eroare: 27-075.2-F. Cap.2.2: Dacă Rd.4 COL.@c ≠ 0, atunci Rd.5 COL.@c trebuie să fie ≠ 0. Valori: R4=@r4, R5=@r5",
                            { "@cuatm": cuatm, "@c": c, "@r4": r4f, "@r5": r5f }
                        )
                    });
                }
                ...
                if (r5f !== 0 && r4f === 0) {
                    webform.errors.push({
                        fieldName: "CAP22_R4_" + col + "_FILIAL",
                        index: j,
                        weight: 19,
                        msg: Drupal.t(
                            "Raion: @cuatm – Cod eroare: 27-075.2-F. Cap.2.2: Dacă Rd.5 COL.@c ≠ 0, atunci Rd.4 COL.@c trebuie să fie ≠ 0. Valori: R5=@r5, R4=@r4",
                            { "@cuatm": cuatm, "@c": c, "@r5": r5f, "@r4": r4f }
                        )
                    });
                }
            }
...
webform.validators.agro24_24 = function (v, allowOverpass) {
    ...
    validate27_056_1(values);
    validate27_029_1(values);
    validate27_054_1(values);
    validate27_054_2(values);
    validate27_055_1(values);
...
//----------------------------------------------
// 27-054.2 (ERROR)
// Tab. 1.1.1: Rd.20 COL.1  <=>  Tab. 2.1: Rd.21 COL.1
// adică: dacă unul ≠ 0 atunci și celălalt ≠ 0 (și invers)

function validate27_054_2(values) {
    var col1 = "C1";
    ...
    if (Rd20_Col1 !== 0 && Rd21_Col1 === 0) {
        webform.errors.push({
            ...
            weight: 19,
            msg: Drupal.t(
                'Cod eroare: 27-054.2. Dacă Tab. 1.1.1, Rând.20 COL.1 ≠ 0, atunci Tab. 2.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R20-C1(@r20), R21-C1(@r21)',
                { '@r20': Rd20_Col1, '@r21': Rd21_Col1 }
            )
        });
    }
    ...
    if (Rd21_Col1 !== 0 && Rd20_Col1 === 0) {
        webform.errors.push({
            ...
            weight: 19,
            msg: Drupal.t(
                'Cod eroare: 27-054.2. Dacă Tab. 2.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.20 COL.1 trebuie să fie ≠ 0. Valori: R21-C1(@r21), R20-C1(@r20)',
                { '@r21': Rd21_Col1, '@r20': Rd20_Col1 }
            )
        });
    }
        ...
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            if (Rd20_Col1_F !== 0 && Rd21_Col1_F === 0) {
                webform.errors.push({
                    ...
                    weight: 19,
                    msg: Drupal.t(
                        'Raion: @cuatm - Cod eroare: 27-054.2-F. Dacă Tab. 1.1.1, Rând.20 COL.1 ≠ 0, atunci Tab. 2.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R20-C1(@r20), R21-C1(@r21)',
                        { '@cuatm': CAP_CUATM_FILIAL, '@r20': Rd20_Col1_F, '@r21': Rd21_Col1_F }
                    )
                });
            }

            // Rd.21 ≠ 0 => Rd.20 ≠ 0 (FILIAL)
            if (Rd21_Col1_F !== 0 && Rd20_Col1_F === 0) {
                webform.errors.push({
                    ...
                    weight: 19,
                    msg: Drupal.t(
                        'Raion: @cuatm - Cod eroare: 27-054.2-F. Dacă Tab. 2.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.20 COL.1 trebuie să fie ≠ 0. Valori: R21-C1(@r21), R20-C1(@r20)',
                        { '@cuatm': CAP_CUATM_FILIAL, '@r21': Rd21_Col1_F, '@r20': Rd20_Col1_F }
                    )
                });
            }
        }
...
function validate27_022(values) {
    ...
    // If Rd.11 COL.8 in Tab. 2.2 ≠ 0, then either Rd.38 COL.1 or Rd.45 COL.1 in Tab. 2.1 must be ≠ 0
    if (Rd11_Col8_Tab22 !== 0 && Rd38_Col1_Tab21 === 0 && Rd45_Col1_Tab21 === 0) {
        webform.errors.push({
            'fieldName': 'CAP21_R38_' + col1,
            'weight': 19,
            'msg': Drupal.t('Cod eroare: 27-022. Dacă Tab. 2.2, Rând.11 COL.8 ≠ 0, atunci Tab. 2.1, Rând.38 COL.1 sau Rând.45 COL.1 trebuie să fie ≠ 0. Valori: R11-C8(@Rd11_Col8_Tab22), R38-C1(@Rd38_Col1_Tab21), R45-C1(@Rd45_Col1_Tab21)', {
    ...
    // If either Rd.38 COL.1 or Rd.45 COL.1 in Tab. 2.1 ≠ 0, then Rd.11 COL.8 in Tab. 2.2 must be ≠ 0
    if ((Rd38_Col1_Tab21 !== 0 || Rd45_Col1_Tab21 !== 0) && Rd11_Col8_Tab22 === 0) {
        webform.errors.push({
            'fieldName': 'CAP22_R11_' + col8,
            'weight': 19,
            'msg': Drupal.t('Cod eroare: 27-022. Dacă Tab. 2.1, Rând.38 COL.1 sau Rând.45 COL.1 ≠ 0, atunci Tab. 2.2, Rând.11 COL.8 trebuie să fie ≠ 0. Valori: R38-C1(@Rd38_Col1_Tab21), R45-C1(@Rd45_Col1_Tab21), R11-C8(@Rd11_Col8_Tab22)', {
                '@Rd38_Col1_Tab21': Rd38_Col1_Tab21,
                '@Rd45_Col1_Tab21': Rd45_Col1_Tab21,
                '@Rd11_Col8_Tab22': Rd11_Col8_Tab22
            })
    ...
    if (values.CAP_NUM_FILIAL) {
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            // If Rd.11 COL.8 FILIAL ≠ 0 in Tab. 2.2, then either Rd.38 COL.1 or Rd.45 COL.1 FILIAL ≠ 0 in Tab. 2.1
            if (Rd11_Col8_Tab22_F !== 0 && Rd38_Col1_Tab21_F === 0 && Rd45_Col1_Tab21_F === 0) {
                webform.errors.push({
                    'fieldName': 'CAP21_R38_' + col1 + '_FILIAL',
                    'index': j,
                    'weight': 19,
                    'msg': Drupal.t('Raion: @CAP_CUATM_FILIAL - Cod eroare: 27-022-F. Dacă Tab. 2.2, Rând.11 COL.8 ≠ 0, atunci Tab. 2.1, Rând.38 COL.1 sau Rând.45 COL.1 trebuie să fie ≠ 0. Valori: R11-C8(@Rd11_Col8_Tab22_F), R38-C1(@Rd38_Col1_Tab21_F), R45-C1(@Rd45_Col1_Tab21_F)', {
                        '@CAP_CUATM_FILIAL': CAP_CUATM_FILIAL,
            ...
            // If either Rd.38 COL.1 or Rd.45 COL.1 FILIAL ≠ 0 in Tab. 2.1, then Rd.11 COL.8 FILIAL ≠ 0 in Tab. 2.2
            if ((Rd38_Col1_Tab21_F !== 0 || Rd45_Col1_Tab21_F !== 0) && Rd11_Col8_Tab22_F === 0) {
                webform.errors.push({
                    'fieldName': 'CAP22_R11_' + col8 + '_FILIAL',
                    'index': j,
                    'weight': 19,
                    'msg': Drupal.t('Raion: @CAP_CUATM_FILIAL - Cod eroare: 27-022-F. Dacă Tab. 2.1, Rând.38 COL.1 sau Rând.45 COL.1 ≠ 0, atunci Tab. 2.2, Rând.11 COL.8 trebuie să fie ≠ 0. Valori: R38-C1(@Rd38_Col1_Tab21_F), R45-C1(@Rd45_Col1_Tab21_F), R11-C8(@Rd11_Col8_Tab22_F)', {
                        '@CAP_CUATM_FILIAL': CAP_CUATM_FILIAL,
                        '@Rd38_Col1_Tab21_F': Rd38_Col1_Tab21_F,
                        '@Rd45_Col1_Tab21_F': Rd45_Col1_Tab21_F,
                        '@Rd11_Col8_Tab22_F': Rd11_Col8_Tab22_F
...
// 27-037
// Tab. 1.1.1: Dacă Rd.11 COL.1 ≠ 0, atunci Rd.21 COL.1 ≠ 0 și invers (ERROR)
...
function validate27_037(values) {
    ...
    // If Rd.11 COL.1 ≠ 0, then Rd.21 COL.1 ≠ 0
    if (Rd11_Col1 !== 0 && Rd21_Col1 === 0) {
        webform.errors.push({
            fieldName: 'CAP111_R21_' + col1,
            weight: 19,
            msg: Drupal.t('Cod eroare: 27-037. Dacă Tab. 1.1.1, Rând.11 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R11-C1(@Rd11_Col1), R21-C1(@Rd21_Col1)', {
                '@Rd11_Col1': Rd11_Col1,
                '@Rd21_Col1': Rd21_Col1
            })
        });
    ...
    // // If Rd.21 COL.1 ≠ 0, then Rd.11 COL.1 ≠ 0
    // if (Rd21_Col1 !== 0 && Rd11_Col1 === 0) {
    //     webform.errors.push({
    //         fieldName: 'CAP111_R11_' + col1,
    //         weight: 19,
    //         msg: Drupal.t('Cod eroare: 27-037. Dacă Tab. 1.1.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.11 COL.1 trebuie să fie ≠ 0. Valori: R21-C1(@Rd21_Col1), R11-C1(@Rd11_Col1)', {
    //             '@Rd21_Col1': Rd21_Col1,
    //             '@Rd11_Col1': Rd11_Col1
    //         })
    //     });
    ...
    if (values.CAP_NUM_FILIAL) {
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            // If Rd.11 COL.1 ≠ 0 in FILIAL, then Rd.21 COL.1 ≠ 0
            if (Rd11_Col1_F !== 0 && Rd21_Col1_F === 0) {
                webform.errors.push({
                    fieldName: 'CAP111_R21_' + col1 + '_FILIAL',
                    index: j,
                    weight: 19,
                    msg: Drupal.t('Raion: @CAP_CUATM_FILIAL - Cod eroare: 27-037-F. Dacă Tab. 1.1.1, Rând.11 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.21 COL.1 trebuie să fie ≠ 0. Valori: R11-C1(@Rd11_Col1_F), R21-C1(@Rd21_Col1_F)', {
                        '@CAP_CUATM_FILIAL': CAP_CUATM_FILIAL,
                        '@Rd11_Col1_F': Rd11_Col1_F,
                        '@Rd21_Col1_F': Rd21_Col1_F
                    })
            ...
            // // If Rd.21 COL.1 ≠ 0 in FILIAL, then Rd.11 COL.1 ≠ 0
            // if (Rd21_Col1_F !== 0 && Rd11_Col1_F === 0) {
            //     webform.errors.push({
            //         fieldName: 'CAP111_R11_' + col1 + '_FILIAL',
            //         index: j,
            //         weight: 19,
            //         msg: Drupal.t('Raion: @CAP_CUATM_FILIAL - Cod eroare: 27-037-F. Dacă Tab. 1.1.1, Rând.21 COL.1 ≠ 0, atunci Tab. 1.1.1, Rând.11 COL.1 trebuie să fie ≠ 0. Valori: R21-C1(@Rd21_Col1_F), R11-C1(@Rd11_Col1_F)', {
            //             '@CAP_CUATM_FILIAL': CAP_CUATM_FILIAL,
            //             '@Rd21_Col1_F': Rd21_Col1_F,
            //             '@Rd11_Col1_F': Rd11_Col1_F
            //         })
...
function validate27_029_1(values) {
    ...
    if (values.CAP_NUM_FILIAL) {
        for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
            ...
            var Rd7_CAP111_F =
                values["CAP111_R7_" + col1 + "_FILIAL"] &&
                    !isNaN(Number(values["CAP111_R7_" + col1 + "_FILIAL"][j]))
                    ? Number(values["CAP111_R7_" + col1 + "_FILIAL"][j])
                    : 0;
